document.addEventListener('DOMContentLoaded', () => {
    ...
    function ensureCanvasSize() {
        ...
        const isPhone = window.innerWidth <= 520;
        const cssTarget = Math.max(240, Math.min(parentW, isPhone ? 360 : 480));
        _cssPx = cssTarget;
    ...
    function onPuzzleSolved() {
        ...
        stopTimer();
        const ok = document.getElementById('success-message');
        if (ok) { ok.style.display = 'block'; ok.textContent = `You win! Time: ${formatTime(elapsed)} — press ↺ Reset for a new puzzle.`; }
    }
